...
namespace sReportsV2.Initializer.OrganizationCSV
{
    public class ImportOrganization
    {
        ...
        public void InsertOrganization(string csvPath, int? countryCD)
        {
            if (organizationDAL.GetAllCount() == 0) 
            {
                ...
                {
                    var partOf = InsertHeadOrganization(groupedOrganizations, j, countryCD);

                    if (groupedOrganizations.ElementAt(j).Count() > 1)
                        InsertSubOrganizations(groupedOrganizations, j, partOf, countryCD);
                }
            }
        }

        private int? InsertHeadOrganization(List<IGrouping<string, CsvOrganization>> groupedOrganizations, int j, int? countryCD) 
        {
        ...
        private void InsertSubOrganizations(List<IGrouping<string, CsvOrganization>> groupedOrganizations, int j, int? partOf, int? countryCD)
        {
            //TO DO IMEPLEMENT 
