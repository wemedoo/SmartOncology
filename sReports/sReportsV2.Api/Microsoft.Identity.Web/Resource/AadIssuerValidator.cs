...
{
    /// <summary>
    /// Generic class that validates token issuer from the provided Azure AD authority. Use the <see cref="AadIssuerValidator"/> to create instances of this class.
    /// </summary>
    public class AadIssuerValidator
    {
        ...
        public static AadIssuerValidator GetIssuerValidator(string aadAuthority)
        {
            ...
            else
            {
                ...
                var aliases = issuerMetadata.Metadata
                    .Where(m => m.Aliases.Exists(a => string.Equals(a, authority, StringComparison.OrdinalIgnoreCase)))
        ...
        {
            if (securityToken is JwtSecurityToken jwtSecurityToken && jwtSecurityToken.Payload.TryGetValue(Microsoft.Identity.Web.ClaimConstants.Tid, out object tenantId))
            {
