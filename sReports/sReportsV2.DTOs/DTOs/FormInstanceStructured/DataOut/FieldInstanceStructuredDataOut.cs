...
namespace sReportsV2.DTOs.Field.DataOut
{
    public partial class FieldDataOut
    {
        ...
        public bool HasAnyValue()
        {
            if (HasAnyFieldInstanceValue)
            {
                for (int i = 0; i < GetRepetitiveFieldCount(); i++)
                {
                    if (HasValue(i))
                    {
                        return true;
        ...
        public List<int> GetRepetitiveFieldInstanceIndexesWithValues()
        {
            List<int> repetitiveFieldInstanceIndexesWithValues = new List<int>();
            for (int repetiveFieldInstanceIndex = 0; repetiveFieldInstanceIndex < GetRepetitiveFieldCount(); repetiveFieldInstanceIndex++)
            {
                if (HasValue(repetiveFieldInstanceIndex))
                {
                    repetitiveFieldInstanceIndexesWithValues.Add(repetiveFieldInstanceIndex);
                }
            }
            return repetitiveFieldInstanceIndexesWithValues;
        }
        ...
        public void AddMissingPropertiesInDependency(FormDataOut form)
        {
            ...
            {
                Dictionary<string, string> fields = form.GetAllFields().GroupBy(x => x.Id).ToDictionary(x => x.Key, x => x.First().Type);
        ...
        public List<FieldConnectedOptionDataOut> GetConnectedFieldDataSource()
        {
            List<FieldConnectedOptionDataOut> fieldConnectedOptions = new List<FieldConnectedOptionDataOut>();
            foreach (FieldInstanceValueDataOut fieldInstanceValue in FieldInstanceValues)
            {
                string value = GetConnectedFieldInstanceValueLabel(fieldInstanceValue);
                if (!string.IsNullOrEmpty(value))
                {
                    fieldConnectedOptions.Add(new FieldConnectedOptionDataOut
                    {
                        FieldInstanceRepetitionId = fieldInstanceValue.FieldInstanceRepetitionId,
                        DisplayValues = value,
                        Values = fieldInstanceValue.Values
                    });
                }
            }

            return fieldConnectedOptions;
        }

        public string GetConnectedFieldInstanceValueLabel(FieldInstanceValueDataOut fieldInstanceValue)
        {
            string value = FormatDisplayValue(fieldInstanceValue, ",");
            return !string.IsNullOrEmpty(value) ? $"{this.Label}: {value}" : string.Empty;
        }
    }
}
