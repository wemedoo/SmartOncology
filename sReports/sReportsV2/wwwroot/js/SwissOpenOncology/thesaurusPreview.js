...
function reloadTable() {
    ...
    checkUrlPageParams();
    setTableProperties(filter, { pageSize: 5, doOrdering: false });
...
function submitThesaurus(event) {
    ...
    let thesaurus = {};
    thesaurus["StateCD"] = $("#state").attr('data-value');
    thesaurus["Id"] = $("#id").val();
...
function setCAndCVocabularyFilterElements(selectedSystems) {
    ...
    for (let selectedSystem of selectedSystems) {
        ...
        $(element).append(getSystemTagCloseSign(selectedSystem));
