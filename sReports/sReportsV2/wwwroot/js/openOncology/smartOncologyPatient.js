...
function getPatientSearchObject() {
    ...
    requestObject['Given'] = $(".patientSearchName").val();
...
$(document).on('focus', '#clinicalTrial', function (e) {
    $(e.target).closest('.column-input').find('img.toggle').hide();
    $(e.target).closest('.column-input').addClass('focus');
});

$(document).on('blur', '#clinicalTrial', function (e) {
    $(e.target).closest('.column-input').find('img.toggle').show();
    $(e.target).closest('.column-input').removeClass('focus');
});
...
function existsTagValue(value, tagType) {
    return $(`.repetitive-values[data-tag='${tagType}']`).find(`.tag-text[data-value='${value}']`).length > 0;
}
...
function submitPatientData(form, event) {
    ...
    if ($(form).valid()) {
        ...
        request['PatientChemotherapyData'] = getPatientChemotherapyData();
...
function getPatientChemotherapyData() {
    var request = {};
    ...
    request['ContraceptionCD'] = $('input[name=contraception]:checked').val();
    ...
    request['DiseaseContextAtInitialPresentationCD'] = $('input[name=diseaseContextAtInitialPresentation]:checked').val();
    request['DiseaseContextAtCurrentPresentationCD'] = $('input[name=diseaseContextAtCurrentPresentation]:checked').val();
