...
namespace sReportsV2.Domain.Services.Implementations
{
    public class FormDAL : IFormDAL
    {
        ...
        {
            return await Collection.AsQueryable().FirstOrDefaultAsync(x => x.Id.Equals(id) && !x.IsDeleted).ConfigureAwait(false);
        }
        ...
        public List<Form> GetAllByOrganization(int organization, int limit, int offset)
        {
            return Collection.AsQueryable()
                .Where(x => !x.IsDeleted && x.OrganizationIds.Contains(organization))
                .OrderBy(x => x.Title)
                .Skip(offset)
                .Take(limit)
                .ToList();
        ...
        public List<Form> GetAllByOrganizationAndLanguage(int organization, string language)
        {
            return Collection
                .AsQueryable()
                .Where(x => !x.IsDeleted)
                .Where(x => x.OrganizationIds.Contains(organization))
        ...
        public Task<List<Form>> GetByTitleForAutoComplete(FormFilterData formFilterData, int page = 1)
        {
        ...
        public async Task<List<Form>> GetByTitle(string title)
        {
            if (!string.IsNullOrWhiteSpace(title))
                return await Collection.AsQueryable().Where(x => !x.IsDeleted && x.Title.ToLower().Contains(title.ToLower())).ToListAsync().ConfigureAwait(false);
            else
                return null;
        }
