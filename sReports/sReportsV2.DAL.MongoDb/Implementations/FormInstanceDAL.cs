    ...
    public class FormInstanceDAL : IFormInstanceDAL
    {
        ...
        public string InsertOrUpdate(FormInstance formInstance, FormInstanceStatus formInstanceStatus)
        {
            ...
            if (formInstance.Id == null)
            {
                ...
                InitOrUpdateChapterPageFieldSetWorkflowHistory(formInstance, formInstanceStatus?.CreatedById);
        ...
        {
            return Collection
                .Find(x => x.EpisodeOfCareRef == episodeOfCareId && x.OrganizationId == organizationId && !x.IsDeleted)
        ...
        public int ReplaceThesaurus(ThesaurusMerge thesaurusMerge, UserData userData = null)
        {
            ...
            filter.ElemMatch(x => x.FieldInstances, c => c.ThesaurusId == thesaurusMerge.OldThesaurus));

            var update = Builders<FormInstance>.Update;
            var fieldsLevelSetter = update.Set("FieldInstances.$.ThesaurusId", thesaurusMerge.NewThesaurus);
            var updatedFieldInstances = Collection.UpdateMany(formInstanceAndFieldThesaurusId, fieldsLevelSetter);
            var filterThesaurus = Builders<FormInstance>.Filter.Eq("ThesaurusId", thesaurusMerge.OldThesaurus);

            var formInstanceSetter = update.Set("ThesaurusId", thesaurusMerge.NewThesaurus);
            var updatedFormInstances = Collection.UpdateMany(filterThesaurus, formInstanceSetter);

            return (int) (updatedFieldInstances.ModifiedCount + updatedFormInstances.ModifiedCount);
        }
        ...
        public async Task<PaginationData<FormInstancePreview>> GetFormInstancesFilteredAsync(FormInstanceFilterData filterData)
        {
            ...
            if (filterData.DoPatientSemanticQuery)
            {
                queryResult = queryResult.Where(x => !x.IsDeleted && x.FieldInstances.Any(x => filterData.ThesaurusIds.Contains(x.ThesaurusId)));
            }
            else
            {
        ...
        public void LockUnlockChapterOrPageOrFieldSet(FormInstancePartialLock formInstancePartialLock)
        {
            ...
            InitOrUpdateChapterPageFieldSetWorkflowHistory(formInstance, formInstancePartialLock.CreateById);
            formInstance.RecordLatestChapterOrPageOrFieldSetChangeState(formInstancePartialLock);
            FindAndReplace(formInstance);
        }
        ...
        public void LockUnlockFormInstance(FormInstanceLockUnlockRequest formInstanceSign)
        {
            ...
            InitOrUpdateChapterPageFieldSetWorkflowHistory(formInstance, formInstanceSign.CreatedById);
            formInstance.RecordLatestWorkflowChangeAndPropagate(formInstance.GetCurrentFormInstanceStatus(formInstanceSign.CreatedById, isSigned: true), new FormInstancePartialLock(formInstanceSign.FormInstanceNextState));
            FindAndReplace(formInstance);
        }
            ...
            switch (filterData.ColumnName)
            {
                ...
                default:
                    ...
                    if (isValidGuid)
                    {
                        ...
                        {
                            var fieldInstance = x.FieldInstances.Find(fi => fi.FieldId == filterData.ColumnName);
        ...
        private void InitOrUpdateChapterPageFieldSetWorkflowHistory(FormInstance formInstance, int? createdById)
        {
            Form form = GetFormDefinition(formInstance.FormDefinitionId);
            formInstance.InitOrUpdateChapterPageFieldSetWorkflowHistory(form, createdById);
        }
        ...
        {
            if (formInstance == null || formInstance.FieldInstances == null || formInstance.FieldInstances.Exists(fi => fi.FieldInstanceValues == null))
                return 0;
        ...
        {
            return GetAllFields(formInstance).Find(x => x.ThesaurusId == thesaurusId);
        }
