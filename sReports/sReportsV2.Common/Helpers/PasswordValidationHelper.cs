...
namespace sReportsV2.Common.Helpers
{
    public static class PasswordHelper
    {
        ...
        public static string AdditionalPasswordChecking(string password, IConfiguration configuration)
        {
            ...
            foreach (PasswordValidationRule validationRule in Enum.GetValues(typeof(PasswordValidationRule)))
            {
                switch (validationRule)
                {
                    case PasswordValidationRule.MinLength:
                        int minPassLength;
                        minPassLength = Int32.TryParse(configuration["MinPassLength"], out minPassLength) ? minPassLength : 4;
                        if (password.Length < minPassLength)
                        {
                            string errorMessage = string.Format("Your password should be longer than {0}.", minPassLength);
